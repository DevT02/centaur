---
name: centaur
description: Read a local repository and apply reviewed edits back to it through Centaur, with every change validated before it is written and an undo snapshot recorded. Use when asked to read, patch, or undo edits in a Centaur workspace, or when the host has no filesystem access of its own.
---
